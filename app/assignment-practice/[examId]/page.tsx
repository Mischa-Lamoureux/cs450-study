...
import { getAssignmentExamById } from '@/data/assignment-practice';
import { ExamQuestion } from '@/data/practice-finals';
import { recordExamScore } from '@/lib/progress';

export default function AssignmentExamTakingPage() {
  const params = useParams<{ examId: string }>();
  const exam = getAssignmentExamById(params.examId);
  ...
  if (!exam) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-8 text-center">
        ...
        <Link href="/assignment-practice" className="text-[var(--primary-light)] hover:underline">
          Back to assignment practice exams
        </Link>
      </div>
    );
  }
  ...
  if (!started) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-8">
        <Link href="/assignment-practice" className="text-sm text-[var(--muted)] hover:text-[var(--foreground)]">
          &larr; All Assignment Practice Exams
        </Link>
